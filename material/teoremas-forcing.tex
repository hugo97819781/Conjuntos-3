...
\thispagestyle{plain}
    ...
    \begin{definicion}
        ...
        El \textbf{lenguaje del forcing} es el lenguaje de primer orden con igualdad y tipo $\{ \in \} \cup M^{B}$, es decir, pensamos cade elemento de $M^B$ como una constante nueva de lenguaje.
    \end{definicion}
    ...
    Hay que tener un poco de cuidado, cada nombre canónico es claramente un nombre, pero el recíproco de esto es falso. Al momento de hacer el universo booleano $M^B$, pueden (y van a) aparecer nuevos nombres. La idea es que $M$ tiene una ``copia isomorfa'' dentro de $M$. Se tiene el siguiente resultado:
    \begin{proposicion}
